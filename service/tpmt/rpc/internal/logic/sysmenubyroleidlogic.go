...
// 通过角色ID获取菜单信息
func (l *SysMenuByRoleIdLogic) SysMenuByRoleId(in *tpmtclient.SysMenuByRoleIdReq) (*tpmtclient.SysMenuByRoleIdResp, error) {
	// 先去中间表找对应的接口IDS
	...
	var list []*tpmtclient.SysMenuListData
	for _, v := range all {
		item, err := l.svcCtx.SysMenuModel.FindOne(l.ctx, v.MenuId)
		if err != nil {
			return nil, err
		}
		list = append(list, &tpmtclient.SysMenuListData{
			Id:          item.Id,                         //菜单ID
	...
	return &tpmtclient.SysMenuByRoleIdResp{
		List: list,
	}, nil
}
