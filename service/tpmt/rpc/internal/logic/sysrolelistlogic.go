...
func (l *SysRoleListLogic) SysRoleList(in *tpmtclient.SysRoleListReq) (*tpmtclient.SysRoleListResp, error) {
	...
	var list []*tpmtclient.SysRoleListData
	for _, item := range all {
		list = append(list, &tpmtclient.SysRoleListData{
			Id:          item.Id,                         //角色ID
			Name:        item.Name,                       //角色名称
	...
	return &tpmtclient.SysRoleListResp{
		Total: count,
		List:  list,
	}, nil
}
