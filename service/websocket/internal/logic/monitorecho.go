...
func (l *Websocket) EchoView(ctx *fasthttp.RequestCtx) {
	...
	err := upgrader.Upgrade(ctx, func(ws *websocket.Conn) {
		...
		defer func() {
			...
			util.MonitorDelSocket(remoteAddr)
			ws.Close()
			global.MonitorUlock.Unlock()
		}()
		...
		// 注册上线
		l.registerForMonitor(ctx, ws, tokenData, device)

		//给客户端发送信息
		err = util.MonitorSendSocket(ws, fmt.Sprintf("register|success|"))
		if err != nil {
		...
		for {
			...
			if l.svcCtx.TaskMaxLimit.Allow() == false {
				err = util.MonitorSendSocket(ws, fmt.Sprintf("error|服务器任务数超过限制请稍后再试"))
				if err != nil {
					logx.Errorf(err.Error())
				}
				continue
			...
			switch data[0] {
			case "ping":
				err = util.MonitorSendSocket(ws, fmt.Sprintf("pong|"))
				if err != nil {
					logx.Errorf(err.Error())
				}
			case "monitor":
				if len(data) > 1 {
					go l.monitorRealData(ctx, ws, data[1], tokenData)
				} else {
					err = util.MonitorSendSocket(ws, fmt.Sprintf("error|发送格式错误请以`功能|内容`格式发送|"))
					if err != nil {
						logx.Errorf(err.Error())
